            ...
            <div class="row food-wrapper">
                ...
                $html="";
                    if(!isset($_GET['category'])) {
                        echo 'Nije izabrana kategorija<br>';
                        exit();
                    }             
                        $idKategorije=$_GET['category'] ;
                        $upit="SELECT idKategorije, nameKategorije FROM category WHERE idKategorije=".$idKategorije;
                        $rez=$db->query($upit);
                        while($red=$db->fetch_object($rez)){
                            // echo " <a href='breakfest.php?kategorija=$red->idKategorije'>$red->nameKategorije</a>";
                            $html .= "<h2>$red->nameKategorije</h2>"; 
                        }
                        echo $html;
                ...
                <?php 
                        $html="";
                        $upit="SELECT * FROM view_headlinetwo WHERE idKategorije=".$idKategorije;
                        $rez=$db->query($upit);
                        while($red=$db->fetch_object($rez)){
                            $html .= "<div class='col-md-3 col-6 mb-10  archive-post'>";
